...
bench() {
    ...
    OUT=benchmarks/macro/$1/results
    echo executing $1 using $2 workers $3, $(date) | tee -a ${IN}/run.res
    mkdir -p ${OUT}
    EXEC="${RUN_CMD} --file ${IN}/streaming.js --threads $2 --servers $2 --warmup_rounds 1 $4" 
    echo ${EXEC} > run.sh
    echo $1: $({ bash run.sh > ${OUT}/run_$2$3;  } 2>&1; tail -n 1 ${OUT}/run_$2$3;  sort -g -k 1,1  ${OUT}/run_$2$3 > r1; mv r1 ${OUT}/run_$2$3; rm -f r1;) | tee -a ${IN}/run.res
...
do_bench() {
    # reset the file 
    rm -f benchmarks/macro/$1/run.res
    touch benchmarks/macro/$1/run.res
