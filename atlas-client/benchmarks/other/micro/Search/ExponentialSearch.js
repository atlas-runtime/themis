...
function binarySearch (arr, x, floor, ceiling, value) {
  // Middle index
...
function exponentialSearch (arr, length, value) {
  ...
  return binarySearch(arr, value, i / 2, Math.min(i, length), value)
}


export function ExponentialSearch_bench(size){
  let arr = [];
  for(let i=0;i<size;i++){
      arr[i]=i;
  }
  exponentialSearch(arr, arr.length, 4)
  return "bench_done"
}
